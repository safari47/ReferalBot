...
@connection()
async def acess_open(session, user_id: int, acess: bool):
    try:
        user = await session.scalar(select(User).filter_by(telegram_id=user_id))
        if user:
            user.acess = acess
        ...
        logger.error(f"Ошибка при изменении доступа: {e}")
        await session.rollback()
...
async def get_all_users(session):
    try:
        ...
        for user in user_list:
            telegram_id, first_name, last_name, username, refer_id = map(
                str,
                (
                    user.telegram_id,
                    user.first_name,
